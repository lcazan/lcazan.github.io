    ...
    <style>
        ...
        * { box-sizing: border-box; touch-action: manipulation; }
        ...
        body, html { 
            ...
            overflow: hidden; 
            /* THE FIX: Flexbox pushes everything to the bottom */
            display: flex;
        ...
        .app-container {
            width: 100%;
            height: 100vh; /* Keeps original proportions */
            position: relative;
            /* Slide the entire container down */
        ...
        .history-item.selected { color: #000 !important; background: #33ffcc; box-shadow: 0 0 15px rgba(51, 255, 204, 0.6); border-radius: 4px; }
        ...
        .grid { 
            ...
            gap: 6px; 
            padding: 10px; 
            /* Add bottom safe area padding so buttons aren't hidden by gesture bars */
        ...
        .shift-active { background: #ff8c00 !important; }
...
<script>
    // [Script remains the same as your saved version]
    ...
    function triggerHaptic() { if (navigator.vibrate) navigator.vibrate(2); }
    ...
    function renderEntry() {
        ...
        document.getElementById('ansBtn').innerText = shiftMode ? 'SCI' : 'ANS';
        document.getElementById('acBtn').innerText = shiftMode ? 'CLR HIST' : 'CLR';
    ...
    function handleInput(val) {
        ...
        if (val === 'up') {
            if (historyIndex < items.length - 1) { historyIndex++; items[historyIndex].scrollIntoView(); }
        } else if (val === 'down') {
            if (historyIndex > -1) { historyIndex--; if (historyIndex === -1) viewport.scrollTop = viewport.scrollHeight; }
        } else if (val === '=') {
            if (historyIndex !== -1) {
                let v = items[historyIndex].getAttribute('data-ans');
                entryArr.splice(cursorIdx, 0, ...v.toString().split(''));
                cursorIdx += v.toString().length; historyIndex = -1;
            } else solve();
        } else if (val === 'ac') {
    ...
    function solve() {
        try {
            ...
            fullHistory.push({expr, res: lastAnswer});
